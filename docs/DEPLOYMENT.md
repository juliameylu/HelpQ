...
The backend is automatically deployed via GitHub Actions when CI passes on
`main`.

Environment variables are set in the Azure portal under **HelpQ Web App →
...
Verify the backend is live: `https://helpq.azurewebsites.net/health` Should
return `{ "status": "ok" }`.
...
The frontend is automatically built and deployed via GitHub Actions after CI
succeeds on `main`.

Environment variables are set in the Azure portal under **HelpQ Static Web App →
