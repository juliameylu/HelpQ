...
describe("validateUuid", () => {
  ...
  test("returns error for UUID with wrong format (missing hyphens)", () => {
    expect(validateUuid("550e8400e29b41d4a716446655440000", "id")).toMatch(
...
describe("validateRequiredTrimmedString", () => {
  ...
  test("returns null when string equals maxLength exactly", () => {
    expect(
      validateRequiredTrimmedString("abcde", "name", { maxLength: 5 })
    ).toBeNull();
  });
  ...
  test("returns error when value is not a string (number)", () => {
    expect(validateRequiredTrimmedString(42, "name")).toMatch(
      /must be a string/
    );
  });

  test("returns error when value is not a string (null)", () => {
    expect(validateRequiredTrimmedString(null, "name")).toMatch(
      /must be a string/
    );
  });

  test("returns error when value is not a string (undefined)", () => {
    expect(validateRequiredTrimmedString(undefined, "name")).toMatch(
      /must be a string/
    );
  });

  test("returns error when value is not a string (array)", () => {
    expect(validateRequiredTrimmedString([], "name")).toMatch(
      /must be a string/
    );
  });
  ...
  test("returns error when trimmed string exceeds maxLength", () => {
    ...
    expect(
      validateRequiredTrimmedString(long, "name", { maxLength: 255 })
    ).toMatch(/at most 255/);
  });

  test("error message includes field name for maxLength violation", () => {
    const msg = validateRequiredTrimmedString("a".repeat(10), "title", {
      maxLength: 5
    });
  ...
  test("trims before checking maxLength", () => {
    ...
    expect(
      validateRequiredTrimmedString("  hi  ", "name", { maxLength: 5 })
    ).toBeNull();
  });
});
